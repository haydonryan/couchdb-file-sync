{
  description = "Nix packaging for couchdb-file-sync (prebuilt app-bin and from-source app-src)";
  ...
  outputs = { self, nixpkgs }:
    ...
    in
    {
      ...
      devShells = forAllSystems (system:
        ...
        in
        {
          # Focused shell for hacking on the project: Rust toolchain plus the
          # pkg-config/cc bits needed to build native crates. Every C-adjacent
          # dependency (bundled sqlite via rusqlite) is self-contained, so no
